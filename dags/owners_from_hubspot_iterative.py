...
def fetch_all_owners():
    ...
    # print(f's3_hook Before: {s3_hook}')
    # s3_hook.extra_params = {'Config': {'read_timeout': 120}}
    # print(f's3_hook After: {s3_hook}')
    print("Connection with s3 build successfully")

    # S3 bucket and file info
    bucket_name = "zubairtests3ci"
    s3_key = "required_columns_owners.json"
    retries = 3
    delay_sec = 30
...
def fetch_owners_json_from_s3(**kwargs):
    ...
    s3_hook.load_string(
        ...
        key='required_columns_owners.txt',
        bucket_name='zubairtests3ci',
        replace=True
    )
...
s3_file_name = 'required_columns_owners.txt'
bucket_name = 'zubairtests3ci'
...
with DAG(
    dag_id = 'Fetch_Owners_min_properties_final',
